...
'''
__version__ = '2.1.0'
__mod_name__ = 'FixDate'
__description__ = 'Correct and standardize dates'
__url__ = 'https://brightedge.co.za'
__author__ = 'Hendrik du Toit'
__author_email__ = 'hendrik@brightedge.co.za'
__license__ = 'Bright Edge eServices'
...
class FixDate:
    ...
    def __init__(
        ...
        '''Description'''
        self.logger_name = '{}.{}'.format(p_parent_logger_name, __mod_name__)
...
def do_tests(p_app_path='', p_cls=True):
    ...
    def basic_test(p_app_path=p_app_path):
        ...
        success = True
        date = FixDate(__mod_name__, '  .  .  ', p_in_format='DMY')
        if date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '.  .', p_in_format='DMY')
        if date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '11.10.03', p_in_format='DMY')
        if date.date != datetime.date(2003, 10, 11):
            success = success and False
        date = FixDate(__mod_name__, '11.10.68', p_in_format='DMY')
        if date.date != datetime.date(1968, 10, 11):
            success = success and False
        date = FixDate(__mod_name__, '68.10.11', p_in_format='YMD')
        if date.date != datetime.date(1968, 10, 11):
            success = success and False
        date = FixDate(__mod_name__, '10.11.68', p_in_format='MDY')
        if date.date != datetime.date(1968, 10, 11):
            success = success and False
        date = FixDate(__mod_name__, '073 339 5214')
        if date.date_str is not None and date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '19681011')
        if date.date != datetime.date(1968, 10, 11):
            success = success and False
        date = FixDate(__mod_name__, '1968/10/11')
        if date.date != datetime.date(1968, 10, 11):
            success = success and False
        date = FixDate(__mod_name__, '4/9/1996')
        if date.date != datetime.date(1996, 9, 4):
            success = success and False
        date = FixDate(__mod_name__, '2001/9/8')
        if date.date != datetime.date(2001, 9, 8):
            success = success and False
        date = FixDate(__mod_name__, '5/10/1976')
        if date.date != datetime.date(1976, 10, 5):
            success = success and False
        date = FixDate(__mod_name__, '2000/4/21')
        if date.date != datetime.date(2000, 4, 21):
            success = success and False
        date = FixDate(__mod_name__, '1968/10/1')
        if date.date != datetime.date(1968, 10, 1):
            success = success and False
        date = FixDate(__mod_name__, '20-Oct-1968')
        if date.date != datetime.date(1968, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '20-Oct-68')
        if date.date != datetime.date(1968, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '20/10/1968')
        if date.date != datetime.date(1968, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '00/00/00')
        if date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '00-00-00')
        if date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '2017-01-0I')
        if date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '2017-02-30')
        if date.date != datetime.date(2017, 2, 28):
            success = success and False
        date = FixDate(__mod_name__, '')
        if date.date is not None:
            success = success and False
        date = FixDate(__mod_name__, '2017-20-10')
        if date.date != datetime.date(2017, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '2017.20.10')
        if date.date != datetime.date(2017, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '20.10.2017')
        if date.date != datetime.date(2017, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '10.20.2017')
        if date.date != datetime.date(2017, 10, 20):
            success = success and False
        date = FixDate(__mod_name__, '1900/00/00')
        if date.date is not None:
            success = success and False
        au.result_rep(success, 'Done')
        return success

    # end basic_test

    success = True
    t_apputils = au.AppUtils(
        __mod_name__, __version__, __description__, p_app_path=p_app_path, p_cls=p_cls
    )
    ...
    t_apputils.print_header(p_cls=p_cls)
    success = basic_test()
    t_apputils.result_rep(success, 'Done')
    t_apputils.print_footer()
    if success:
        return t_apputils.archive_path
    return False
